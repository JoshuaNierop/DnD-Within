...
### Nog open
- [!] P2 — #fATDUg Portret **crop/edit-knop** (character-image-widget + wizard) — vereist browser-geteste sessie (interactieve editor). UPLOAD bestaat al in beide contexten (wizard-sidebar-portrait label-upload #5Y8aZh; image-widget `upload-portrait`-knop wg-render.js:422 in editValuesMode). Ontbreekt: een EDIT/crop-affordance.
  - **Aanpak-voorstel**: sla crop-params op het portret op (focal point `objX%/objY%` + `zoom`), niet de pixels (cover/slice rendert al → focal-point = `object-position` voor HTML, x/y/scale-offset voor SVG `<image>`). Editor = klein modal/overlay met live preview + **knop-gebaseerde** controls (zoom +/−, nudge ↑↓←→ of 2 range-sliders) i.p.v. drag → veel minder coördinaten-risico, makkelijker blind-correct. Eén editor-component, hergebruikt vanuit wizard (HTML-context) én image-widget (open via een edit-knop naast upload).
  - **BROWSER-TEST nodig**: live preview-mapping focal→object-position (HTML) vs SVG `<image>` viewport-offset; opslaan/sync van crop-params; toepassen bij render in beide contexten.
